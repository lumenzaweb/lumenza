...
const ActionButton = ({ href, title, icon, bgColor }) => (
  <motion.a
    ...
    className="flex items-center justify-end" // Content is aligned to the right
    variants={{
      open: { opacity: 1, y: 0, transition: { type: 'spring', stiffness: 300, damping: 24 } },
      closed: { opacity: 0, y: 20, transition: { duration: 0.2 } },
    }}
  >
    {/* Label is on the LEFT and is HIDDEN on mobile (md:block) */}
    <span className="mr-4 text-sm font-medium text-gray-700 bg-white px-4 py-2 rounded-full shadow-sm whitespace-nowrap hidden md:block">
      {title}
    </span>
...
const FloatingContactButtons = () => {
  ...
  return (
    // Component is now fixed to the BOTTOM-RIGHT
