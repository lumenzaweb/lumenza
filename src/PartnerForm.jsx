...
const isValidEmail = (email) => {
    // Basic regex for email format: checks for text@text.text
...
// --- INLINED MINIMAL COMPONENTS (Kept for completeness) ---

// Minimal SEO Component
const SEO = ({ title, description }) => {
  useEffect(() => {
    document.title = title || "Partner Form";
    document.querySelector('meta[name="description"]')?.setAttribute('content', description || "Apply to partner with us.");
  }, [title, description]);
  return null;
};
...
const AnimatedBubbleBackground = () => (
    <div className="absolute inset-0 overflow-hidden">
        {/* Placeholder for visual aesthetics */}
        <div className="absolute top-0 left-0 w-64 h-64 bg-red-800/10 rounded-full blur-3xl animate-pulse opacity-20"/>
        <div className="absolute bottom-1/4 right-0 w-80 h-80 bg-red-600/15 rounded-full blur-3xl animate-pulse delay-500 opacity-20"/>
    </div>
);
...
const PartnerForm = () => {
  ...
  }, [formData.state, formData.country]); // Added formData.country as dependency here
  ...
  const validateStep = (stepToValidate) => {
    ...
    if (stepToValidate === 1) {
        ...
        // --- UPDATED VALIDATION FOR CONTACT NUMBER ---
        if (!formData.contact) {
    ...
    if (stepToValidate === 2) {
        // Since react-select stores an object or null, we check for null
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    // Transform selected objects back to string labels for submission
  ...
  const customSelectStyles = {
    // Ensuring the styles match the dark theme and validation logic
    ...
    menu: (provided) => ({ ...provided, backgroundColor: '#1F2937', borderRadius: '0.75rem', zIndex: 50 }),
  ...
  return (
    <section className="pt-28 sm:pt-32 pb-20 px-4 sm:px-6 bg-gray-900 min-h-screen relative overflow-hidden font-sans">
      ...
      {popup.show && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] backdrop-blur-sm transition-opacity duration-300">
            ...
            <AnimatePresence mode="wait">
              {step === 1 && (
                <motion.div key="step1" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-4">
                  <h3 className="text-2xl font-bold text-white mb-4">Personal & Firm Details</h3>
                  <div>
                  ...
                  <div>
                    <input type="tel" name="contact" placeholder="Your Contact Number (e.g., +1 555 123 4567)" value={formData.contact} onChange={handleChange} className={`w-full p-4 bg-white/5 border rounded-xl placeholder-gray-400 focus:ring-2 focus:ring-red-500 ${errors.contact ? 'border-red-500' : 'border-white/30'}`}/>
                    {errors.contact && <p className="text-red-400 text-xs mt-1 px-1">{errors.contact}</p>}
                  </div>
                  <div>
                    <input type="email" name="email" placeholder="Your Email Address (e.g., user@example.com)" value={formData.email} onChange={handleChange} className={`w-full p-4 bg-white/5 border rounded-xl placeholder-gray-400 focus:ring-2 focus:ring-red-500 ${errors.email ? 'border-red-500' : 'border-white/30'}`}/>
              ...
              {step === 2 && (
                <motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-4">
                   <h3 className="text-2xl font-bold text-white mb-4">Location & Business Details</h3>
                   <div>
              ...
              {step === 3 && (
                <motion.div key="step3" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                  ...
                  <div>
                    <textarea name="message" placeholder="Tell us about your interest or any questions..." value={formData.message} onChange={handleChange} className={`w-full p-4 bg-white/5 border rounded-xl h-40 resize-none placeholder-gray-400 focus:ring-2 focus:ring-red-500 outline-none transition-all duration-300 ${errors.message ? 'border-red-500' : 'border-white/30'}`} />
