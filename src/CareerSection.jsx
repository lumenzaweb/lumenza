      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-24">
            {whyJoinUs.map((item, idx) => (
                <motion.div 
                  key={item.title} 
                  className="bg-gray-900/50 border border-white/10 p-8 rounded-2xl shadow-lg text-center"
                  initial={{ opacity: 0, y: 50 }} 
            ...
            <div className="space-y-4">
                {departments.map((dept) => (
                    <motion.div 
                      ...
                      className="bg-gray-900/50 border border-white/10 rounded-2xl transition-all duration-300 backdrop-blur-sm hover:border-red-500/50"
                      transition={{ layout: { duration: 0.4, ease: "easeOut" } }}
                    >
                        <motion.div layout="position" className="p-6 flex justify-between items-center cursor-pointer">
                        ...
                        <AnimatePresence>
                            {openDepartment === dept.title && (
                                ...
                                >
                                    <div className="border-t border-white/10 pt-4 bg-red-900/20 p-4 rounded-lg">
                                        <h4 className="font-semibold text-red-200 mb-2">Key Responsibilities:</h4>
                                        <ul className="space-y-2 list-disc list-inside text-red-100">
        ...
        >
          <h3 className="text-3xl font-bold mb-6 text-center text-white">Apply Now</h3>
          <form className="grid md:grid-cols-2 gap-6" onSubmit={handleSubmit}>
            ...
            <div className="md:col-span-2 relative">
              <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full p-4 bg-white/5 border border-white/20 rounded-xl text-left flex justify-between items-center">
                  <span className={form.noticePeriod ? "text-white" : "text-gray-400"}>
              ...
              <AnimatePresence>
                  {isDropdownOpen && (
                      <motion.ul initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute z-10 w-full mt-2 bg-gray-800 border border-white/20 rounded-xl shadow-lg">
                          {noticePeriodOptions.map(option => (
                              <li key={option} onClick={() => handleNoticePeriodSelect(option)} className="px-4 py-3 hover:bg-red-500/50 cursor-pointer">{option}</li>
                          ))}
                      </motion.ul>
                  )}
              </AnimatePresence>
